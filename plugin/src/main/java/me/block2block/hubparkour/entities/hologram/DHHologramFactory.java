...
public class DHHologramFactory extends HologramFactory {

    private final Map<Integer, List<IHologram>> holograms = new HashMap<>();
    private static DHHologramFactory instance;

    public static DHHologramFactory getInstance() {
        return instance;
    }

    public DHHologramFactory() {
        instance = this;
    }

    @Override
    public IHologram createHologram(IParkour parkour, String name, Location location) {
        IHologram hologram = new DHHologram(parkour, name, location);
